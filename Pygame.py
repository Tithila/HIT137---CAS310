...
ENERGY_ORB_GAIN = 4
ENEMY_PROJECTILE_DAMAGE = 5  # Damage dealt by enemy projectiles
...
class Player(pygame.sprite.Sprite):
    ...
    def draw_info(self, screen):
        ...
        kills_text = font.render(f"Kills: {total_kills}", True, BLACK)  # Display total kills
        screen.blit(ammo_text, (10, 40))
        screen.blit(energy_text, (10, 70))
        screen.blit(kills_text, (10, 100))  # Position for kills text
        if self.ammo == 0:
...
class Orb(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, orb_type):
        super().__init__()
        self.orb_type = orb_type  # 'health' or 'energy'
...
class Boss(Enemy):
    def __init__(self, level):
        ...
        self.speed = 0  # Boss stays in place at the right edge
        self.rect.midright = (SCREEN_WIDTH - 50, GROUND_HEIGHT - 50)  # Slightly above the ground

    def draw_health_bar(self, screen):
        # Health bar settings
        bar_x, bar_y = SCREEN_WIDTH // 4, 20
        bar_width = SCREEN_WIDTH // 2  # Fixed width to fit within the screen
        bar_height = 20
        
        # Calculate the current width based on boss's health, capped by bar_width
        max_health = ENEMY_BASE_HEALTH * BOSS_HEALTH_MULTIPLIER
        current_width = min(bar_width, int(bar_width * (self.health / max_health)))
        
        # Draw the health bar background (black) and current health (red)
